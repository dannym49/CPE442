...
void neonSobel(const cv::Mat& grayOutput, cv::Mat& sobelOutput, int startRow, int endRow) {
    for (int i = startRow + 1; i < endRow - 1; i++) {  //Padding
        ...
        for (; j <= grayOutput.cols - 8; j += 8) {
            ...
            for (int m = -1; m <= 1; m++) {
                const unsigned char* rowPtr = grayOutput.ptr<unsigned char>(i + m);
        ...
        for (; j < grayOutput.cols - 1; j++) {
            ...
            for (int m = -1; m <= 1; m++) {
                ...
                for (int n = -1; n <= 1; n++) {
                    unsigned char pixelValue = rowPtr[j + n];
            ...
            unsigned char* sobelRowPtr = sobelOutput.ptr<unsigned char>(i - 1);
            sobelRowPtr[j - 1] = static_cast<unsigned char>(sum);
        }
    }
}
