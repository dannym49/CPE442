...
//                 // Load pixels and convert to signed 16-bit integers
//                 uint8x8_t pixels_u8 = vld1_u8(&rowPtr[j - 1]);
//                 int16x8_t pixels = vreinterpretq_s16_u16(vmovl_u8(pixels_u8));
//                 vGradient = vmlaq_n_s16(vGradient, pixels, verticalFilter[m + 1][0]);
//                 hGradient = vmlaq_n_s16(hGradient, pixels, horizontalFilter[m + 1][0]);

//                 pixels_u8 = vld1_u8(&rowPtr[j]);
//                 pixels = vreinterpretq_s16_u16(vmovl_u8(pixels_u8));
//                 vGradient = vmlaq_n_s16(vGradient, pixels, verticalFilter[m + 1][1]);
//                 hGradient = vmlaq_n_s16(hGradient, pixels, horizontalFilter[m + 1][1]);

//                 pixels_u8 = vld1_u8(&rowPtr[j + 1]);
//                 pixels = vreinterpretq_s16_u16(vmovl_u8(pixels_u8));
//                 vGradient = vmlaq_n_s16(vGradient, pixels, verticalFilter[m + 1][2]);
//                 hGradient = vmlaq_n_s16(hGradient, pixels, horizontalFilter[m + 1][2]);
//             }
...
//     // Apply grayscale with NEON intrinsics
//     neonGrayscale(*input, *grayOutput, startRow, endRow);

//     // Apply Sobel filter with NEON intrinsics
